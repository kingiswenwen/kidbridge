...
function generateAddition10(): Question {
  ...
  const formats = [`${num1} + ${num2} = ?`, `${num2} + ${num1} = ?`];
...
function generateAddition20(): Question {
  ...
  return {
    ...
    visual:
      answer <= 15 ? { type: "sticks", count: answer } : undefined,
  };
}
...
function generateCompare(): Question {
  ...
  const formats = [`${num1} ○ ${num2}`, `比较 ${num1} 和 ${num2}`];
...
const wordProblemTemplates: Array<{
  template: (n1: number, n2: number) => string;
  type: "add" | "sub";
}> = [
  { template: (n1, n2) => `小明有 ${n1} 个苹果，妈妈又给了他 ${n2} 个，现在有几个？`, type: "add" },
  { template: (n1, n2) => `花园里有 ${n1} 朵红花，又开了 ${n2} 朵，现在有几朵？`, type: "add" },
  { template: (n1, n2) => `停车场有 ${n1} 辆汽车，又开来 ${n2} 辆，一共几辆？`, type: "add" },
  { template: (n1, n2) => `教室里有 ${n1} 个小朋友，又来了 ${n2} 个，现在有几个？`, type: "add" },
  { template: (n1, n2) => `小红有 ${n1} 支铅笔，爸爸又买了 ${n2} 支，现在有几支？`, type: "add" },
  { template: (n1, n2) => `草地上有 ${n1} 只蝴蝶，又飞来 ${n2} 只，现在有几只？`, type: "add" },
  { template: (n1, n2) => `树上有 ${n1} 只小鸟，飞走了 ${n2} 只，还剩几只？`, type: "sub" },
  { template: (n1, n2) => `小红有 ${n1} 颗糖果，吃掉了 ${n2} 颗，还有几颗？`, type: "sub" },
  { template: (n1, n2) => `盘子里有 ${n1} 个橘子，小明吃了 ${n2} 个，还剩几个？`, type: "sub" },
  { template: (n1, n2) => `书架上有 ${n1} 本书，借走了 ${n2} 本，还剩几本？`, type: "sub" },
  { template: (n1, n2) => `妈妈买了 ${n1} 个鸡蛋，用了 ${n2} 个做饭，还有几个？`, type: "sub" },
  { template: (n1, n2) => `公交车上有 ${n1} 人，下车了 ${n2} 人，还有几人？`, type: "sub" },
  { template: (n1, n2) => `气球有 ${n1} 个，飞走了 ${n2} 个，还剩几个？`, type: "sub" },
];

function generateWordProblem(): Question {
  const problem =
    wordProblemTemplates[randomInt(0, wordProblemTemplates.length - 1)];
...
function generateCounting(): Question {
  ...
  const formats = ["数一数，有几个？", "下面一共有多少个？", "请数一数图中有几个？"];
  ...
  return {
    ...
    visual: { type: "fruits", items, count },
  };
}
